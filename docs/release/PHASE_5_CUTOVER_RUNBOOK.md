...
- Confirm Linux procfs (or `ps` on non-Linux hosts) and `lsof` are available for fail-closed PID, command, working-directory, file-holder, and loopback-port validation.
